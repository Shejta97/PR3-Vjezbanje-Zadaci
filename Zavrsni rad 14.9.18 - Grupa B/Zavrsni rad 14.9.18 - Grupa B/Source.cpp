...
struct ZavrsniRad {
	...
	void DodajPoglavlje(const char *naziv, const char* sadrzaj)
	{
		for (int i = 0; i < _trenutnoPoglavlja; i++)
		{
			if (strcmp(_poglavljaRada[i]->_naslov, naziv) == 0)
			{
				...
				int size = strlen(_poglavljaRada[i]->_sadrzaj) + strlen(sadrzaj) + 2;
				char *temp = new char[size];
		...
		temp[_trenutnoPoglavlja]->Unos(naziv, sadrzaj);
		_poglavljaRada = temp;
		_trenutnoPoglavlja++;
	}
	...
	void Kopiraj(ZavrsniRad z) {
		...
		for (int i = 0; i < z._trenutnoPoglavlja; i++)
		{
...
struct Nastavnik {
	...
	bool DodajZavrsniRad(ZavrsniRad rad)
	{
		...
		{
			if (_teme[i] != nullptr && (strcmp(_teme[i]->_tema, rad._tema) == 0 || strcmp(_teme[i]->_brojIndeksa, rad._brojIndeksa) == 0))
				return false;
		...
		for (int i = 0; i < max_zavrsnih; i++)
		{
			...
			if (_teme[i] != nullptr && strcmp(_teme[i]->_brojIndeksa, brojindeksa) == 0)
			{
				...
				for (int j = 0; j < _teme[i]->_trenutnoPoglavlja; j++)
				{
					...
					if (!_teme[i]->_poglavljaRada[j]->_prihvaceno)
						return nullptr;
...
tuple<char *,int,float> PronadjiStudenta(Nastavnik *n, int v)
{
	...
	for (int i = 0; i < v; i++)
	{
		...
		for (int j = 0; j < max_zavrsnih; j++)
		{
			...
			{
				sumaSvihOcjenaZavrsnihRadova += n[i]._teme[j]->_konacnaOcjena;
				brojZavrsnihRadova++;
			}
		}
